...
#Region "Metodos Privados"
    ...
    Public Sub _prMostrarRegistro(_N As Integer)
        ...
        With grVentas
            ...
            _CodEmpleado = .GetValue("ydnumi")
            tbcodigovendedor.Text = .GetValue("taven")
            ...
            If (Not IsNothing(frmAyuda.filaSelect)) Then
                ...
                If (Not _fnExisteProductoConLote(numiProd, "20170101", CDate("2017/01/01"))) Then
                    ...
                    Else
                        ...
                        ToastNotification.Show(Me, "El producto no tiene Stock para continuar la venta".ToUpper, img, 2000, eToastGlowColor.Red, eToastPosition.BottomCenter)
                    End If
                    'b.yfcdprod1, a.iclot, a.icfven, a.iccven
    ...
    End Sub
    Public Sub _MostrarCalcularPorcentaje()
        Dim montodesc As Double = tbMdesc.Value
        Dim pordesc As Double = Math.Round((montodesc * 100) / grdetalle.GetTotal(grdetalle.RootTable.Columns("tbtotdesc"), AggregateFunction.Sum))
        tbPdesc.Value = pordesc

    End Sub
    Public Sub _prCalcularPrecioTotal()
        Dim montodesc As Double = tbMdesc.Value
        'Dim pordesc As Double = Math.Round(((montodesc * 100) / grdetalle.GetTotal(grdetalle.RootTable.Columns("tbtotdesc"), AggregateFunction.Sum)), 0)
        montodesc = Math.Round((tbPdesc.Value * grdetalle.GetTotal(grdetalle.RootTable.Columns("tbtotdesc"), AggregateFunction.Sum) / 100), 2)
        'tbPdesc.Value = pordesc
        ...
        If (gb_FacturaIncluirICE = True) Then
            tbtotal.Value = Math.Round((grdetalle.GetTotal(grdetalle.RootTable.Columns("tbtotdesc"), AggregateFunction.Sum) - montodesc + tbIce.Value), 2)
        Else
            tbtotal.Value = Math.Round((grdetalle.GetTotal(grdetalle.RootTable.Columns("tbtotdesc"), AggregateFunction.Sum) - montodesc), 2)
        End If
    ...
    Public Sub _GuardarNuevo()
        ...
        ef.Header = "¿Esta seguro que quiere grabar la venta?".ToUpper
        ef.ShowDialog()
